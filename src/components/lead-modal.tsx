  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-5 right-5 z-30 grid size-16 place-items-center rounded-full bg-[#002D62] text-center text-xs font-bold text-white shadow-xl ring-4 ring-white hover:bg-[#001B3D]"
        ...
        >
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <div className="mb-5 flex items-start justify-between">
              <div>
                <p className="text-sm font-bold text-[#F5A623]">MIDA PROPERTY</p>
                <h2 className="text-2xl font-extrabold text-[#002D62]">นัดชมโครงการ</h2>
            ...
            {status === "success" ? (
              ...
              <form onSubmit={submit} className="space-y-3">
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 p-3 outline-none focus:border-[#002D62]"
                />
                <input
                  name="phone"
                  required
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 p-3 outline-none focus:border-[#002D62]"
                />
                <select name="budget" className="w-full rounded-xl border border-slate-200 p-3 text-slate-600">
                  <option value="">งบประมาณที่สนใจ</option>
                ...
                <button disabled={status === "sending"} className="button-primary w-full disabled:opacity-60">
                  {status === "sending" ? "กำลังส่ง..." : "ส่งคำขอนัดชม"}
                </button>
              </form>
            )}
          </div>
